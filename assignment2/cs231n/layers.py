...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_hat = (x - sample_mean) / np.sqrt(sample_var + eps)       # shape (N, D)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    x, x_hat, mu, sigma2, gamma, beta, eps = cache
    m = x.shape[0]

    dgamma = np.sum(dout * x_hat, axis=0)
    dbeta = np.sum(dout, axis=0)
    dx_hat = dout * gamma
    dsigma2 = np.sum(dx_hat * (x - mu) * (-0.5) * (sigma2 + eps) ** (-1.5), axis=0)
    dmu = np.sum(dx_hat * (-1) / np.sqrt(sigma2 + eps), axis=0) + dsigma2 * np.mean((-2) * (x - mu), axis=0)
    dx = dx_hat / np.sqrt(sigma2 + eps) + dsigma2 * 2 * (x - mu) / m + dmu / m
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(Hp):
        for j in range(Wp):
            input = x[:, :, i*stride:(i*stride+HH), j*stride:(j*stride+WW)]  # (N, C, HH, WW)
            out[:, :, i, j] = np.amax(input, axis=(2, 3))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    #     for j in range(Wp):
    #         input = x[:, :, i*stride:(i*stride+HH), j*stride:(j*stride+WW)]  # (N, C, HH, WW)
    #         input_reshaped = input.reshape(N*C, HH*WW)
    #         index = input_reshaped.argmax(axis=1)
    #         dx[range(N*C), index] += dout[:, :, i, j].reshape([-1])
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    N, C, H, W = x.shape
    x_vec = x.reshape([N, -1])
    gamma_vec = np.repeat(gamma, H*W)
    beta_vec = np.repeat(beta, H*W)
    out, cache = batchnorm_forward(x_vec, gamma_vec, beta_vec, bn_param)
